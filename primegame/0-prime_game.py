...
def is_prime_sieve(n):
    """
    Generates a list of the number of prime
    numbers up to each number from 0 to n.
    Args:
        n (int): The maximum number up to
                which to generate prime counts.

    Returns:
        List[int]: A list where the value at
                  each index i is the count of prime
...
def isWinner(x, nums):
    ...
    Args:
        ...
        where each value represents n for a round.

    Returns:
        str or None: The name of the player who won the
